...
In this case we say that the metric on $\bm{X}$ \textit{defines} a \emph{convergence} $\spc{X}_n\z\GHto \spc{X}_\infty$ in the \emph{sense of Gromov--Hausdorff}.
...
Show that there is a covergence $(\lambda\cdot \spc{L},p)\z\GHto (\T_p,0)$ as $\lambda\to\infty$.
Moreover, ???
\end{thm}
