...
\parbf{Compact case.}
The Gromov--Hausdorff convergence of compact spaces has specially nice properties.
...
By the last exercise, there is a metric on the set of isometry classes of compact metric spaces
such that $\dist{\spc{X}_n}{\spc{X}_\infty}{}\to0$ if and only if there is a Gromov--Hausdorff convergence $\spc{X}_n\to\spc{X}_\infty$.
This metric space will be denoted by $\GH$. 
Given two compact metric $\spc{X}$ and $\spc{Y}$, define
\[\dist{\spc{X}}{\spc{Y}}{\GH}=\inf\set{\eps>0}{\spc{X}\le \spc{Y}+\eps
\quad\text{and}\quad
\spc{Y}\le \spc{X}+\eps}.\]
This is not the standard Gromov--Hausdorff metric, but it defines the same topology.

\begin{thm}{Exercise}\label{ex:complete}
Show that $\GH$ is complete.
\end{thm}
...
It remains to apply \ref{ex:complete} and \ref{clm:compact-net}.
