...
In this lecture, we prove the line splitting theorem and apply it to the study tangent spaces of Alexandrov spaces.
...
It can be thought of as the shifted distance function from the ideal point of $\gamma$.
...
It is obvious that if $\spc{X}=A\oplus B$ then $\spc{X}$ is isometric to $A\times B$.
...
It proves the needed inequality in  the case $s=0$.
...
is $2$-concave.
In particular, $\phi(t)\le t^2+at+b$ for some constants $a,b\in\RR$; see \ref{Function comparison}.
Therefore,
\[
...
Suppose  \ref{SHORT.opposite} holds. We can assume that $|u|=|v|\ne 0$ since otherwise \ref{SHORT.<x,u>} is trivial. Since  $\mangle(u,v)=\pi$  and $\T_p$ is a metric cone it follows that $\RR_{\ge 0}\cdot u\cup \RR_{\ge 0}\cdot u$ is a line. 
Recall that $\T_p$ has nonnegative curvature.
Therefore, by \ref{ex:adjacent-CBB} it holds that  $\mangle\hinge 0ux+\mangle\hinge 0vx=\pi$;
%the proof is the same as \ref{ex:adjacent-CBB}.
...
Then $S$ is a dense G-delta set \footnote{ i.e. a countable intersection of open sets}.
...
Halbeisen's example \cite{alexander-kapovitch-petrunin2024,halbeisen} shows that compactness of the space of directions is essential in the proof that space of directions is $\pi$-geodesic (see \ref{thm:finite-space-of-directions}).
...
In \ref{sec:packing-numbers} and \ref{sec:all-dim}, we will show that linear dimension of Alexandrov spaces coincides with all reasonable notions ofdimension;
...
Choose small $r>0$ and $y_i\in [px_i]$ such that $\dist{p}{y_i}{}=r$ for each $i$.
...
By construction, for all large $n$, we have $\mangle\hinge pz{m_n}\approx0$  with arbitrarily small given error.
...
Show that for some fixed $r>0$ and $\lambda$, the function $h_{\bm{y}}$ is $\lambda$-concave in $\oBall(p,r)$,
\begin{enumerate}[(i)]
...
Let $t_0(\bm{y})=\tfrac{1}{\eps^2}\cdot\bigl|f(p)-\bm{y}\bigr|$.
Use \ref{lem:fg-dist-est} to show that the map
\[\map\:{\bm{y}}\mapsto \alpha_{\bm{y}}\circ t_0(\bm{y})\]
is continuous (in fact H\"older) in $\Omega=\oBall(F(p),\tfrac{\eps^2}{2}\cdot r )\subset\RR^m$
...
one of the inequalities holds in each of the following two strings of $m$  inequalities
\begin{align*}
...
Recall that $\gamma_x^+(0)=\log_p x$ (here $\log_p\:\spc{A}\to \T_p$ is a multivalued function; so, $\gamma_x^+(0)$ might be not the only value of $\log_p x$).
...
The first part is also known as \index{Bishop's inequality}\emph{Bishop's inequality}.
...
The latter implication was proved by Grigory Perelman and the third author \cite{perelman-petrunin:qg} using gradient exponential map. The proof we give in this book is different and relies on the right-inverse theorem (\ref{thm:right-inverse}).
