...
Note that the right inverse theorem implies that if $\LinDim\spc{L}\ge m$, then $\spc{L}$ contains a subset homeomorphic to an open ball in $\EE^m$.
In particular $\TopDim\spc{L}\ge m$, where $\TopDim$ for the \index{Lebesgue coverning dimension}\emph{Lebesgue coverning dimension}.
By Szpilrajn's theorem \cite[theorems V 8 and VII 2]{hurewicz-wallman}, $\HausDim\spc{L}\ge\TopDim\spc{L}$,
where $\HausDim$ stands for Hausdorff dimension.
Finally, by the Bishop--Gromov inequality, we get that $\LinDim \spc{L}\ge \HausDim\spc{L}$.
