...
Let $\spc{A}$ be an $\Alex0$ space.
Show that $\LinDim \spc{A}\z\ge m$ if and only if for some $m+2$ points $p$, $a_0,\z\dots, a_{m}\in \spc{A}$
we have
...
where \index{51@$\pack_\eps\spc{X}$}$\pack_\eps\spc{X}$ is the exact upper bound on the number of points $x_1,\z\dots,x_k\in \spc{X}$ such that $\dist{x_i}{x_j}{}\ge\eps$ if $i\ne j$.
