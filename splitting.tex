...
\parit{Proof of \ref{thm:splitting}.} Consider two Busemann functions, $\bus_+$ and $\bus_-$, associated with half-lines $\gamma:[0,\infty)\to \spc{L}$ and $\gamma:(-\infty,0]\to \spc{L}$ respectively; that is,
\[
\bus_\pm(x)
\df
...
Let $\spc{L}$ be an Alexandrov space.
We define its \index{linear dimension}\emph{linear dimension} \index{$\LinDim$}$\LinDim \spc{L}$ as the least upper bound on integers $m$ such that 
...
after that, we will use \index{$\dim$}$\dim\spc{L}$ for $\LinDim\spc{L}$.

\begin{thm}{(\textit{n}+1)-comparison}\label{thm:n+1}
Let $\spc{L}$ be an $\Alex0$ space.
Then for any finite set of points $p,x_1,\dots,x_n\in \spc{L}$, there exists a model configuration 
...
\parit{Proof.}
Suppose $\LinDim \spc{L}=\infty$.
By the right-inverse theorem (\ref{thm:right-inverse}), $\spc{L}$ contains a compact subset $K$ with an arbitrarily large $\TopDim K$.
In particular,
\[\TopDim\spc{L}=\infty.\] 
...
Now suppose $\LinDim \spc{L}=m<\infty$.
By the Bishop--Gromov inequality (\ref{inq:BG} and \ref{ex:BG}), 
\[\HausDim\spc{L}\le m.\]

Since $\spc{L}$ is proper (\ref{ex:diam-compact:proper}),
Szpilrajn's theorem, implies that
\[\TopDim\spc{L}\le \HausDim\spc{L}\le m.\]
Finally, the right-inverse theorem (\ref{thm:right-inverse}) implies that $m\le\TopDim\spc{L}$.
\qeds
