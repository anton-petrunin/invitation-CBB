...
Let $\spc{A}$ be an Alexandrov space.
We define its \index{linear dimension}\emph{linear dimension} \index{50@$\LinDim$}$\LinDim \spc{A}$ as the least upper bound of integers $m$ such that
...
Show that there is a ball $B=\cBall[p,r]_{\spc{A}}$ and a constant $c>0$ such that $\pack_\eps B\ge \tfrac c{\eps^m}$ for any $\eps>0$.
\end{subthm}

\begin{subthm}{ex:tangent=Em:G-delta}
Show that $\T_p\spc{A}\iso \EE^m$ for points $p$ in a dense G-delta set of points in $\spc{A}$.
\end{subthm}
...
In the general case, by \ref{ex:tangent=Em:G-delta}, we can find a point $p'$ arbitrarily close to $p$ such that $\T_{p'}\iso \EE^k$ for some $k\le m$.
