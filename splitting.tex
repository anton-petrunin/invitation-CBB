...
In this lecture, we prove the line splitting theorem and apply it in tangent spaces of Alexandrov spaces.
...
\[\bus_+(x)+\bus_-(x)\ge 0,\]
where $\bus_+$ and $\bus_-$ are the Busemann functions associated with half-lines $\gamma:[0,\infty)\to \spc{X}$ and $\gamma:(-\infty,0]\to \spc{X}$ respectively.
\end{thm}
...
Since $\mangle(u,v)=\pi$ and $\T_p$ is a metric cone, the half-lines in directions of $u$ and $v$ form a line in $\T_p$.
