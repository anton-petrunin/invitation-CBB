...
The following exercise adds \index{Minkowski dimension}\emph{Minkowski dimension} (also known as \index{box-counting dimension}\emph{box-counting dimension}) to the list of reasonable dimensions discussed in Section~\ref{sec:all-dim}.

\begin{thm}{Exercise}\label{ex:box-counting}
Let $\spc{A}$ be an Alexandrov space.
Given a point $p\in \spc{A}$ and $R>0$, let $m(p,R)$ be the least upper bound on values $\alpha>0$ such that $\eps^\alpha\cdot\pack_\eps \cBall[p,R]\to 0$ as $\eps\to 0$.

Show that $m(p,R)=\LinDim \spc{A}$; in particular, $m(p,R)$ is an integer, and it does not depend on the choice of $p$ and $R$.
\end{thm}
