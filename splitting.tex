...
Let us define its \index{linear dimension}\emph{linear dimension} $\LinDim \spc{L}$ as the least upper bound on integers $m$ such that 
...
Moreover, if $\LinDim\spc{L}=m$, then $f$ defines a bi-Lipschitz embedding of a small neighborhood of $p$.
\end{thm}

If $\LinDim\spc{L}=m$, then the restriction of $f$ to a neighbohood provided by the second part of theorem is called \index{distance chart}\emph{distance chart} at $p$.
The following exercise will guide you thru the proof.

\begin{thm}{Exercise}\label{ex:proof-right-inverse}
Suppose $p,a_0,\dots,a_m\in\spc{L}$ and $f\:\spc{L}\to\RR$ are as in \ref{thm:right-inverse} and $\eps>0$.
...
Show that $f_{\bm{y}}$ is $\lambda$-concave in $\oBall(p,r)$ for some $r>0$.
Moreover,
\begin{enumerate}[(i)]
\item\label{111} $(\dd_q\distfun_{a_i})(\nabla_q f_{\bm{y}})<-\tfrac{1}{10}\cdot\eps^2$ if $\dist{a_i}{q}{}>y_i$ and
\item\label{222} $(\dd_q\distfun_{a_i})(\nabla_q f_{\bm{y}})>\tfrac{1}{10}\cdot\eps^2$ if 
\[\dist{a_i}{q}{}-y_i=\min_j\{\dist{a_j}{q}{}\z-y_j\}<0.\]
\end{enumerate}
...
continuous in $\oBall(\dist{\bm{a}}{p}{},\tfrac{\eps^2\cdot r}{20} )\subset\RR^m$
and finish the proof of the first statement in \ref{thm:right-inverse}.
\end{subthm}

\begin{subthm}{ex:proof-right-inverse:dim}
Show that there is $\eps>0$ such that one of the following $m$ inequalities hold
\begin{align*}
\mangle\hinge xy{a_1}&<\tfrac\pi2-\eps,\quad\dots,\quad \mangle\hinge xy{a_m}<\tfrac\pi2-\eps,
\\
\mangle\hinge yx{a_1}&<\tfrac\pi2-\eps,\quad\dots,\quad\mangle\hinge yx{a_m}<\tfrac\pi2-\eps
\end{align*}
...
Note that as a corollary of \ref{thm:right-inverse} we get that \textit{all reasonable definitions of dimension give the same result for Alexandrov spaces}.
...
where $\TopDim$ and $\HausDim$ stands for \index{Lebesgue coverning dimension}\emph{Lebesgue coverning dimension} and \index{Hausdorff dimension}\emph{Hausdorff dimension}, respectively.
\end{thm}

%??? лучше доказать --- утверждение про топ.размерность используется в \ref{thm:top-bry}!!!
