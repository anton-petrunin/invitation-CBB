...
In \ref{sec:packing-numbers} and \ref{sec:all-dim}, we will show that linear dimension of Alexandrov space coincides with all reasonable dimensions;
...
If $\spc{A}$ is an $\Alex{\pm1}$ space, then the same holds if we exchange $\EE^m$ to $\SSS^m$ or $\HH^m$, respectively; in the $\Alex{1}$ case one has to assume in addition that $\diam \spc{A}\le \pi$.
\end{thm}
...
and it can be done essentially the same way, one only has to exchange $\EE^{m-1}$ to $\HH^{m-1}$.
\qeds
