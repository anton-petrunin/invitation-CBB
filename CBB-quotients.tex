...
In the $\Alex1$ case, the proof produces a slightly weaker statement: \textit{$\SSS^2$-comparison holds for a quadruple $p$, $x_1$, $x_2$, $x_3$ in the quotient of $\Alex1$ if $\dist{p}{x_i}{}<\tfrac\pi 2$ for each $i$}.
In particular, the quotient space is \textit{locally} $\Alex1$.
But since $\Alex1$ space is geodesic, so is its quotient.
