...
In the $\Alex1$ case, the proof produces a slightly weaker statement: \textit{{if $\sigma\:\spc{A}\to\spc{B}$ is a submetry and $\spc{A}$ is $\Alex1$ then $\SSS^2$-comparison holds for any quadruple $p$, $x_1$, $x_2$, $x_3$ in $\spc{B}$ if $\dist{p}{x_i}{}<\tfrac\pi 2$ for each $i$}}.
...
Indeed, since the action is properly discontinuous, the subgroup of $\Gamma$ that fix $A_F$ pointwise has to be finite.
...
For example, each face of an $m$-dimensional cube is its primitive extremal subset;
