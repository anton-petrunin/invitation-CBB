...
But since an $\Alex1$ space is geodesic, so is its image under a submetry (prove it!).
...
This symmetry has two fixed points, the north pole and the south pole, that correspond to the orbits of $(1,0)$ and $(0,1)$ in $\SSS^3$.
\end{subthm}
...
Denote by $T(r)$ the inverse image $S(r)$ in $\SSS^3$, and let $a(r)$ be its area.
...
Find a bound on number of extremal points in an $\Alex{-1}$ space in terms of its dimension $m$ and diameter $D$.
\end{thm}
...
The extremal subsets of Alexandrov space were briefly discussed in \ref{sec:bry-remarks}.
...
Also, every vertex, edge, face of a cube, and their unions, are extremal subsets of the cube.
