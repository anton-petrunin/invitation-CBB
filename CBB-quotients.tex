...
Find a bound on number of extremal points in an $\Alex{-1}$ space in terms of its dimension $m$ and diameter.
\end{thm}
...
The extremal subsets of Alexandrov space were bristly discussed in \ref{sec:bry-remarks}.
...
An nonempty extremal set is called \index{primitive extremal set}\emph{primitive} if it contains no proper extremal subsets with nonempty relative interior.
(For every point of Alexandrov space the set of points reachble by recurcive application of gradient flow for squares of distance functions forms a primitive extremal set.)
...
\begin{thm}{Conjecture}
Any $m$-dimensional compact $\Alex0$ space has at most $3^m$ minimal extremal subsets.
\end{thm}
