...
Recall that a group action $G\acts\spc{X}$ is called \index{effective action}\emph{effective} if for any $g\in G$ there is $x\in\spc{X}$ such that $g\cdot x\ne x$.
...
Then there is a distance noncontracting map $\Sigma\to \tfrac12\cdot \SSS^2$, where $\tfrac12\cdot \SSS^2$ denotes the standard 2-sphere rescaled with a factor $\tfrac12$.
\end{thm}
...
Show that
\[b_{p,q}(r)=2\cdot \pi\cdot\sqrt{(p\cdot \sin r)^2+(q\cdot \cos r)^2}.\]
\end{subthm}
...
By \ref{lem:S^3/S^1}, each angle $\mangle\hinge{y_i}{y_j}{y_k}\le \tfrac\pi2$ for any three distinct points
$y_i$, $y_j$, $y_k$.
(Alternatively this follows from the fact that each  point $y_i$ must  extremal in $\spc{A}$ by similar argument as in \ref{lem:extr/G} (see \cite[4.2]{perelman-petrunin} for details) and hence the diameter of  $\Sigma_{y_i}\spc{A}$ is at most $\pi/2$. )
...
\index{properly discontinuous}\emph{properly discontinuous} (that is, for any compact set $K\subset \EE^m$ and $x\z\in \EE^m$ there are only finitely many elements $g\in \Gamma$ such that $g\cdot x\in K$) and \index{cocompact action}\emph{cocompact} (that is, the quotient space $\spc{A}=\EE^m/\Gamma$ is compact).
...
Some none-sharp explicit estimates on the number of extremal subsets can be obtained from the ideas in the proof of Gromov's Betti number theorem \ref{thm:betti}, add a reference to Fujioka "Uniform boundedness on extremal subsets in Alexandrov spaces"  here.
