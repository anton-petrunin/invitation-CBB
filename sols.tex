...
\parbf{\ref{ex:tangent=Em}}; \ref{SHORT.ex:tangent=Em:ball}.
Apply the right-inverse theorem and \ref{ex:resporka}.

\parit{\ref{SHORT.ex:tangent=Em:G-delta}.}
Let $B=\cBall[p,r]$ be as in \ref{SHORT.ex:tangent=Em:ball}, so given a radius $R\gg r$ we can choose $\eps>0$ and an integer $N$ such that
\[\pack_\eps \cBall(0,R+r)_{\EE^{m-1}}< N\le \pack_\eps B.\]

Applying \ref{cor:euclid-subcone} to an $\eps$-packing $x_1,\dots,x_N$, we get that $\dim\Lin_s\z\ge m$ for any point $s$ in a dense G-delta subset of $\oBall(p,R)$.
Conclude that $\dim\Lin_s\z\ge m$ for any point $s$ in a dense G-delta subset of $\spc{A}$ and apply \ref{ex:dim-max}.
...
\parbf{\ref{ex:box-counting}.}
Apply \ref{ex:resporka} and \ref{thm:right-inverse} to show that $m(p,R)\ge \LinDim \spc{A}$.
To prove that $m(p,R)\le \LinDim \spc{A}$, reuse the argument in the Bishop--Gromov inequality.
