...
Suppose the configuration $p$, $x_0,\z\dots, x_{m}\in \spc{L}$ meets the condition.
By \ref{ex:grad-dist} the directions $\dir q{x_0},\z\dots,\dir q{x_m}\in \Lin_q$ for G-delta dense set of points $q\in \spc{L}$.
If $q$ is sufficiently close to $p$, then $\angk q{x_i}{x_j}>\tfrac\pi2$,
and therefore, $\mangle\hinge q{x_i}{x_j}>\tfrac\pi2$ for $i\ne j$.
...
Observe that there is a subemetry $\sigma_n$ such that the inverse image $\sigma_n^{-1}\{y\}$ is a union of the arcs from the original $\SSS^1$-orbits.
