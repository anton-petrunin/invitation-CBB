...
Let us denote by $|v|$ the norm of $v\in\spc{W}$.
It is sufficient to check that the norm satisfies the parallelogram identity
\[2\cdot |v|^2+2\cdot |w|^2=|v-w|^2+|v+w|^2\]
for any $v,w\in \spc{W}$.

Fix nonzero $v,w\in \spc{W}$.
Then $\gamma\:t\mapsto t\cdot v$ and $\sigma\:s\mapsto s\cdot w$ are constant-speed geodesics.
By assumption, there is a well-defined angle measure $\alpha$ between them, so
\[\alpha=\lim_{t,s\to 0+} \angk 0{t\cdot v}{s\cdot w}.\]

Setting $t=s$, we get that  $\angk 0{t\cdot v}{t\cdot w}=\const$.
Hence $\alpha=\angk 0vw$ and
 \[\cos \alpha=\frac{|v|^2+|w|^2-|v-w|^2}{2\cdot|v|\cdot |w|}.\]

The same argument with $s=2\cdot t$ implies that
\[\cos \alpha=\frac{|v|^2+4\cdot |w|^2-|v-2\cdot w|^2}{4\cdot |v|\cdot |w|}.\]
These two equalities imply the parallelogram identity for $w$ and $v-w$.
Since $v$ and $w$ are arbitrary, we get the result.
...
\parit{Comment.} Alternatively the result follows from existence of angles in Alexandrov spaces and \ref{ex:norm-angles}.
