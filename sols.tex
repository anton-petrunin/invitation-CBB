...
Since space is proper, it is separable; 
that is, we can choose an countable everywhere dense set $\{x_1,x_2,\dots\}$.

Let $A_1,A_2,\dots$ be a sequence of closed set.
Applying the diagonal procedure, we can pass to subsequence such that for each $i$ the sequence $\distfun_{A_n}x_i$ converges as $n\to\infty$;
...
Note that $z_\infty$ is a midpoint of $x_\infty$ and $y_\infty$ and apply Menger's lemma (\ref{lem:mid>geod}).
...
By \ref{ex:perim-k>0}, perimeter of any triangle in $\spc{X}$ is at most $2\cdot\pi$.
We need to check $\EE^2$-comparison for a given quadruple of points $q,y_1,y_2,y_3$ in $\Cone\spc{X}$.
We can assume that non of these points is the origin, otherwise perturb them a bit.

Set $x_i=y_i/|y_i|$ for each $i$ and $p=q/|q|$; we will assume that $p,x_1,x_2,x_3$ lie in $\spc{X}$, which is the unit sphere in $\Cone\spc{X}$.
...
Finally if some of the model triangles is not defined, consider rescaling of $\spc{X}$ with a coefficient $\lambda$ slightly smaller than 1.
...
\parit{Comment.}
The last part on the proof is close to the argument in \ref{thm:CBB-closed}.
...
Therefore we can choose $\lambda$ and $r>0$ so that $f_{\bm{y}}$ is $\lambda$-concave in $\oBall(p,r)$.
...
\end{align*}
If $\dist{a_i}{x}{}>y_i$, then $\dd_xf_{\bm{y}}=\eps\cdot \dd_x\distfun_{a_0}$, or
\[\dd_xf_{\bm{y}}
=
\min_{i\in S} \{\,\dd_x\distfun_{a_j}\,\}+\eps\cdot \dd_x\distfun_{a_0}\]
for some nonempty subset $S\subset \{1,\dots,m\}\setminus\{i\}$.
Furthermore, 
...
Then
\[\dd_x f_{\bm{y}}\le \dd_x \distfun_{a_i}{}{}+\eps\cdot(\dd_p\distfun_{a_0}{}{}).\] 
...
\parit{\ref{SHORT.ex:proof-right-inverse:end}} Straightforward application of \ref{lem:fg-dist-est} and a reformulation of \ref{SHORT.ex:proof-right-inverse:alpha}.
...
Reuse the argument in the first part of the proof of Bishop--Gromov inequality.
...
\parit{\ref{SHORT.ex:pack-vol:dim}} By \ref{ex:dim-lim} $\dim\spc{L}_\infty\le m$.
...
\parbf{\ref{ex:Hilbert/G}.} Read \cite[Section 4]{terng-thorbergsson} and/or solution for ``Quotient of the Hilbert space'' in \cite{petrunin2020}.
