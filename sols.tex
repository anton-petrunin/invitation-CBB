...
\parbf{\ref{ex:k-><mono}.}
Let us write the Riemannian metric on $\MM(\kappa)$ in the polar coordinates $(\theta,r)$;
it has form 
$(\begin{smallmatrix}
h^2&0
\\
...
Suppose $\kappa<\Kappa$, consider the map $\MM(\kappa)\to\MM(\Kappa)$ that sends a point to the point with the same polar coordinates.
...
Show that $\SSS^3$ can be identified with the unit sphere in $\CC^2$
so that the the $\SSS^1$-action is given by matrices
$\left(
\begin{smallmatrix}
u^p&0\\
0& u^q
\end{smallmatrix}
\right)$
where $u$ is a unit complex number and $(p,q)$ is a pair of relatively prime integers.
Since the action has no fixed points, we also have that $p\ne0$ and $q\ne 0$.

The quotient $\SSS^3/\SSS^1$ is a homeomorphic to the 2-sphere and it has $\SSS^1$ symmetry.
The metric on the space is completely defined by the length of $\ell(r)$ of a circle with radius $r$ centered at one pole

If $p=q=1$, then this actions defines the Hopf fibration.
In this case the quotient is the round 2-spere or radius $\tfrac12$.

Suppose 

can be extended to the standard action of torus,
which is induced by multiplication of diagonal matrix 
$\left(
\begin{smallmatrix}
e^{i\cdot u}&0\\
0&e^{i\cdot v}
\end{smallmatrix}
\right)$
on a unit sphere $\SSS^3$ in $\CC^2$.

Conclude that the quotient $\SSS^3/\SSS^1$ has $\SSS^1$-simmetry.
It is can be described by a Riemannian on $\SSS^2$ with two singularities at the poles.
In polar coordinates $(\theta,r)$ this Riemannian metric can be written as $\left(
\begin{smallmatrix}
h^2&0\\
0&1
\end{smallmatrix}
\right)$,
where $h=h(r)$.
For Hopf's $\SSS^1$-action; that is, if it is given by
$\left(
\begin{smallmatrix}
e^{i\cdot t}&0\\
0&e^{i\cdot t}
\end{smallmatrix}
\right)$
we have $h=\sin (2\cdot r)$.
We need to show that for any other action $h(r)\le\sin (2\cdot r)$.
???
...
We need to show that if a polyhedral surface is $\CBB(0)$, then the total angle $\theta$ of any of its vertex $p$ it at most $2\cdot\pi$.
...
The latter contradicts $\CBB(0)$ comparison. 
...
\parit{Comments.}
More generally, if a sequence of geodesic $\CBB(\kappa)$ spaces $\spc{L}_1,\spc{L}_2,\dots$ converges to $\spc{L}_\infty$ and $\dim \spc{L}_n=\dim \spc{L}_\infty<\infty$ for any $n$,
