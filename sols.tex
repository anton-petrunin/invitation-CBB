...
\parbf{\ref{ex:tangent=Em+}.}
Recall that by \ref{ex:tangent=Em}, we have a G-delta dense set of points $p\in \spc{A}$ such that $\T_p\iso \EE^{k(p)}$ for some integer $k(p)\le m$.
Try to apply the Bishop--Gromov inequality (\ref{inq:BG} and \ref{ex:BG}) and Theorem~\ref{thm:dim=dim} to show that $k(p)=m$ for any $p$ in the set.
