...
Applying a diagonal procedure, we can pass to a subsequence such that for each $i$ the sequence $\distfun_{A_n}x_i$ converges as $n\to\infty$;
...
Without assuming that all $A_i$ are contained in a compact subset of $\spc{X}$ the statement is false as can be seen by the example of $\space{X}=\RR, A_n\z=\{0,n\}, A_\infty=\{0\}$.
...
Alternatively, if there are two distinct geodesics from $z$ to $x$ then their concatenations with $[yz]$ gives bifurcating geodesics from $y$ to $x$. This contradicts \ref{ex:0-angle}.
...
Observe that two half-lines in $\Cone\spc{X}$ that start from the origin and go in directions $x$ and $y\in\spc{X}$ form a line if and only if $\dist{x}{y}{\spc{X}}\ge \pi$.
Apply the splitting theorem or non-branching of geodesics in  $\Cone\spc{X}$ to show that for any $x\in \spc{X}$ there is at most one point $y$ such that $\dist{x}{y}{\spc{X}}\ge \pi$ and in this case we have equality.
...
$\vol_m\:\bm{K}_m\to \RR$ is continuous \cite[10.8]{burago-gromov-perelman}.

\parbf{\ref{ex:barrier}.}
Choose a smooth upper barrier $\bar f$ of $f$ at $t_0$ with $\bar f''\le \lambda_1$, provided by Lemma \ref{lem:barrier}.
Observe that the composition $\phi\circ\bar f$ is an upper barrier of $\phi\circ f$ at $t_0$.
It remains to apply the chain rule to $\phi\circ\bar f$ and use the lemma again.

\parbf{\ref{ex:pack-sphere}.}
Choose a ball $B=\cBall[x,R]$ such that $\cBall[x,2\cdot R]\subset \Omega$.
...
Let $r=k\cdot \eps$, for each $q_i\in B_k$ choose $q_i'\in [pq_i]$ such that $\dist{p}{q_i'}{}=r$.
...
Checking the curvature bounds is easy once you realize that $(\SSS^3,g)$ is isometric to a quotient space $(\SSS^3\times (\eps\cdot \SSS^1))/\SSS^1$, where $\SSS^1$ acts diagonally by the Hopf action on $\SSS^3$ and the standard action on $\SSS^1$.
...
By the double suspension theorem due to Edwards, %maybe give a reference here?
$\Susp^2\Sigma\cong\SSS^5$.
...
The first inequality follows from the generalized Kiszbraun theorem proved by Urs Lang and Viktor Schroeder \cite{lang-schroeder:kirszbraun, alexander-kapovitch-kirszbraun, alexander-kapovitch-petrunin2024};
...
This way we get a convex triangle $\tilde\Lambda$ with two straight sides $b$ and $c$ and a polygonal side of total length $a$.
...
This is a consequence of the following obsercation: $\langle x-\bar x,\bar y-\bar x\rangle\le 0$ and $\langle y-\bar y,\bar x-\bar y\rangle\le 0$ for any $x,y\in \EE^m$.
...
Show that geodesics between the essential vertices can be chosen so that they do not cross each other;
that is, every pair of geodesics intersects only at the common vertex.
In this case they divide the surface into plane triangles.
...
Therefore the curve $\bar\gamma$ lies on the surface of $K$, has the same ends as $\gamma$, and satisfies \[\length\bar\gamma<\length\gamma.\]
