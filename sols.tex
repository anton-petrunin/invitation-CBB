...
Consider a sequence of geodesics $[p\,\gamma(t_n)]$.
...
Observe that $f_n$ is is $\tfrac1{\lambda_n}$-concave and $f_n\to \dd_pf$ as $n\to\infty$ for the convergence $(\lambda_n\cdot \spc{A},p)\z\to (\T_p,0)$ from \ref{ex:GHto-tangent-finite-dim}.

Choose a geodesic $\gamma$ in $\T_p$; assume it can be exteded behind its endpoints.
Construct a sequence of geodesics $\gamma_n$ in $\lambda_n\cdot \spc{A}$ that converge to $\gamma$ (here you should use that $\gamma$ can be extened).
...
In particular, \textit{every infinite-dimensional Alexandrov space $\spc{A}$ contains a bi-Hölder copy of Euclidean ball of arbitrary dimension}.
...
From above, we can choose an index $i>0$ so that $\mangle\hinge xy{a_i}<\tfrac\pi2-\eps$ (if $\mangle\hinge yx{a_i}<\tfrac\pi2-\eps$, then swap $x$ and $y$).
...
Follow Case~3 in the proof \ref{thm:hsiang-kleiner} to glue a train-like space from copies of $\spc{A}$ using two of these components.
...
Use it to show that union of two convex polygons can triangulated by convex triangles.
...
Conclude that these triangles form faces of tetrahedron (possibly degenerate to a quadrangle)
