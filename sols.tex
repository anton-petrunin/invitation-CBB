...
Denote by $k$, $l$, and $m$ the number of vertices, edges, and triangles, respectively in a chosen triangulation of $P$.
Note that
\[2\cdot l=3\cdot m
\qquad\text{and}\qquad
k-l+m=\chi(P).\]
...
Consider the function $f\:t\mapsto \dist{\gamma_1(0)}{\gamma_1(t)}{}$.
Show that $f'(t)\ge \cos\omega$ is the left-hand side is defined.
Make a conclusion.

(With a bit more work, one can improve the inequality to
\[\cos \omega\cdot \length \gamma_1\le \length \gamma_0.)\]

\parit{\ref{SHORT.ex:approximation:excess}.} Show that the total angle around each vertex of the triangulation is at most $2\cdot\pi$ and argue as in \ref{ex:gauss-bonnet}.

\parit{\ref{SHORT.ex:approximation:length}.} Apply the angle comparison.
...
$\mangle\hinge p{y_1}{y_2}_{\EE^3}=\mangle\hinge p{x_1}{x_2}_P$.
...
Conclude that the function $t\mapsto \dist{q}{y_i(t)}{W}$ is nonincreasing for any $q\in P$.
Therefore, 
\begin{align*}
\dist{y_1}{y_2}{W}
&=\dist{y_1(0)}{y_2(0)}{W}=
...
&\ge\dist{x_1}{x_2}{P}.
\end{align*}
The last inequality follows since the closest point projection $W\to P$ is short.
...
Denote by $x_1'\in [px_1]_{P}$ and $x_2'\in [px_2]_{P}$ the corresponding points;
that is, $\dist{p}{y_1'}{\EE^3}=\dist{p}{x_1'}P$ and $\dist{p}{y_2'}{\EE^3}=\dist{p}{x_2'}P$.
From  above, we have that $\dist{y_1'}{y_2'}{\EE^3}\ge\dist{x_1'}{x_2'}P$;
in other words, 
\[\angk p{y_1'}{y_2'}\ge \angk p{x_1'}{x_2'};\]
here we think of  $[p{y_1'}{y_2'}]$ as a triangle in $\EE^3$, but $[p{x_1'}{x_2'}]$ as a triangle in $P$.
Note that 
