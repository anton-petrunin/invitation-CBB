...
\parit{Comments.} This example is called \index{Berger's sphere}\emph{Berger's spheres}.
...
Repeat the proof of \ref{thm:extr-point} for fixed $\alpha=\tfrac1{10}$ showing that any unit ball in $\spc{A}$ has at most $1000^m$ extramal points.
...
Show that the quotient space $\Delta=\spc{A}/\mathbb{S}^1$ is an $\Alex1$ disc and $\gamma$ projects isometrically to its boundary $\partial\Delta$.
...
The first inequality follows from the generalized version of Kiszbraun's theorem proved by Urs Lang and Viktor Schroeder \cite{lang-schroeder:kirszbraun, alexander-kapovitch-kirszbraun,alexander-kapovitch-petrunin2024};
