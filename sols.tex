...
In particular, we can canonically and isometrically dentify  $\T_p\spc{A}$ with $\T_{\iota_n(p)}\spc{A}_n$.
...
\end{align*}
Hence the statement follows by adding these inequalities.
...
\parit{Remark.} The same argument more generally shows that given a (pointed) Gromov-Hausdorff convergence of geodesics $\Alex{\kappa}$ spaces $\spc{X}_n\to \spc{X}$ and $\lambda$-concave functions $f_n$ on $\spc{X}_n$ pointwise converging to a function $f$ on  $\spc{X}$ the limit function $f$ is also $\lambda$-concave.
...
it was proved by Grigory Perelman and the third author \cite{perelman-petrunin} and generalizes a theorem of Joseph Liberman \cite{liberman} about geodesics on convex surfaces.

\parbf{\ref{ex:native}.}
The "only if" direction easily follows from the fact that $f\circ\proj\circ\gamma$ is invariant under the canonical isometric involution of $\spc{W}$.
...
Let $\tilde{\spc{P}}$ be the polyhedral surface obtained from $\spc{P}$ as in the proof of  \ref{thm:approximation}
...
\parbf{\ref{ex:surface-covergence}.} We will use that the nearest-point projection $\Pi$ from the Euclidean space $\EE^m$  to a convex body $C$ is short;
that is, distance-nonexpanding \cite[13.3]{petrunin-zamora}.  This  easily follows from the fact that for any $x\in \EE^m, y\in C$ it holds that $\langle x-\Pi(x), y-\Pi(x)\rangle\le 0$.
