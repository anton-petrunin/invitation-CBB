...
\parit{Proof.}
To prove the if direction, observe that a near an interior point the surface is locally isometric to a cone over a circle of length $\le 2\cdot\pi$ and near a boundary point it is locally isometric to the cone over an interval of length $\le \pi$.
...
\parit{Construction.}
Let $\spc{P}$ be a closed $\Alex0$ surface.
By part \ref{SHORT.ex:approximation:triangulation}, we can triangulate $\spc{P}$ by small convex triangles, say, diameter of each triangle is less than a given $\delta>0$.
...
If these triangles were convex, then the first estimate in parts \ref{SHORT.ex:approximation:length} and \ref{SHORT.ex:approximation:excess} would imply that $\tilde{\spc{P}}_\delta$ is close to $\spc{P}$ in the sense of Gromov--Hausdorff.
...
Let us define a \index{convex body}\emph{convex body} as a compact convex subset of $\EE^3$ with a nonempty interior.
...
This can be also shown by approximating the defining convex function of a convex body by a smooth convex function using a convolution.
...
Let us denote by $\bm{K}$ the space of all convex polyhedrons in Euclidean space,
...
Choose $p\in K$.
By \ref{ex:liberman} (which is Liberman's lemma), the function $f_p\:t\z\mapsto \distfun_p\circ\gamma(t)$ is semiconcave for any $p\in K$.
...
Suppose a plane $\Pi$ cuts from the surface of a convex body $K$ a disk $\Delta$, and the reflection of $\Delta$ across $\Pi$ lies in $K$.
...
A big part of the theory described in this lecture is generalized to surfaces with bounded integral curvature \cite{alexandrov-zalgaller,reshetnyak1993}.
...
In Cauchy's proof \cite{cauchy}, it was deduced from an analog of the following lemma.
...
It could be considered as a special case of the conjecture about the boundary of Alexandrov space; see \ref{conj:bry}.
