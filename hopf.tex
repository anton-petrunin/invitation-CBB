...
Define \index{rank}\emph{rank} of $\spc{L}$ (briefly, \index{$\rank\spc{L}$}$\rank\spc{L}$) as the minimal value $k$ such that $\spc{L}$ splits isometrically as $\RR^{m-k}\times \spc{K}$;
