...
Define \index{rank}\emph{rank} of $\spc{L}$ (briefly, $\rank\spc{L}$) as the minimal value $k$ such that $\spc{L}\iso\RR^{m-k}\times \spc{K}$,
...
Then we say that $\xi$ is in the \index{space of directions!of a subset}\emph{space of directions} from $p$ to $X$; briefly $\xi\in\Sigma_pX$.

Further, the cone $\T_pX=\Cone(\Sigma_pX)$ will be called \index{tangent space!of a subset}\emph{tangent space} to $X$ at $p$.
