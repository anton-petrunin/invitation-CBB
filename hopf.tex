...
The step of induction is a combination of the implications below.
...
Consider the function $\phi\:\Sigma_x\to\RR$ defined by $\phi(\xi)\df\dd_xf(\xi)$.
...
Otherwise, we can choose a direction $\xi\in\partial\Sigma$ such that
...
Choose such $\xi'(t)$ for $\xi(t)=\dir xz$.
Consider the radial curve $\alpha(s)\df\gexp_x(s\cdot\xi'(t))$.
Let us show that 
\begin{align*}
\dist{p}{z}{\spc{W}}
&\le \dist{p}{\alpha(s)}{\spc{L}_0}+ \dist{\alpha(s)}{z}{\spc{L}_1}\le
\\
&\le \sqrt{\dist[2]{p}{y}{\spc{W}}-2\cdot t\cdot \cos\theta+t^2} =
\\
&=\side\hinge yp{z}.
\end{align*}
for suitable value $s$.
The choice of $s$ comes from the model configuration $\tilde p$, $\tilde x$, $\tilde y$, $\tilde w$, $\tilde z\in \EE^2$
\begin{figure}[ht!]
\vskip-0mm
\centering
...
\end{align*}
where $\phi=\mangle\hinge{x}{y}{z}$ and $\psi=\mangle(\dir xp,\xi')$.
By \ref{prop:gexp}, we get 
\[\dist{p}{\alpha(s)}{\spc{L}_0}+ \dist{\alpha(s)}{z}{\spc{L}_1}\le\dist{\tilde p}{\tilde z}{},\]
and the comparison implies that 
\[\dist{\tilde p}{\tilde z}{}\le \side\hinge yp{z}.\]
