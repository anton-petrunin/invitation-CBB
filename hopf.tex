...
Let us give an inductive definition the boundary for finite-dimensional complete geodesic $\CBB(\kappa)$ space.
...
Suppose $x\mapsto x'$ is a homeomorphism between finite-dimensional geodesic $\CBB(\kappa)$ spaces $\spc{L}$ and $\spc{L}'$. Show that 
...
Let $\spc{L}$ be an $m$-dimensional geodesic $\CBB(\kappa)$ space and $m<\infty$.
...
The doubling $\spc{W}$ of $\spc{L}$ across $\partial \spc{L}$ is a complete geodesic $\CBB(\kappa)$ space.
\end{subthm}
...
By \ref{SHORT.thm:doubling:doubling}$_{m-1}$, the doubling $\Xi$ of $\Sigma_x$ is a geodesic $\CBB(\kappa)$ space.
...
Let us exchange $\gamma\cap \spc{L}_1$ to its reflection in $\spc{L}_0$;
...
If $\gamma(t)\z\in \spc{L}_0\setminus\partial\spc{L}$, then $(f\circ\gamma)''(t)\le 1$ follows from the comparison in $\spc{L}_0$.

Assume $\gamma(t)\in \spc{L}_1\setminus\partial\spc{L}$.
Suppose $[p\gamma(t)]$ crosses $\partial L$ at $x$.
Let $\Sigma_x$ be the space of directions of $\spc{L}$ at $x$
and let $\Xi$ be its doubling.
By \ref{SHORT.thm:doubling:doubling}$_{m-1}$, $\Xi$ is a geodesic $\CBB(1)$ space.
Note that $\dir x{\gamma(t)}$ and $\dir xp$ lie in opposite sides of $\Xi$ and
$\dist{\dir x{\gamma(t)}}{\dir xp}{}\ge \pi$.
Otherwise we could choose a direction $\xi\in\partial\Sigma$ such that
\[\dist{\dir x{\gamma(t)}}{\xi}{}+\dist{\xi}{\dir xp}{}<\pi.\]
...
+\dist{\gamma(t)}{\alpha(s)}{\spc{L}_1}
<\dist{p}{\gamma(t)}{\spc{W}}\]
for small values $s>0$
--- a contradition.

Note that $\Cone \Xi$ contains a line with directions $\dir x{\gamma(t)}$ and $\dir xp$.
...
$\xi$ and $\xi'$ lie on a geodesic $[\dir x{\gamma(t)} \dir xp]_\Xi$.

Choose $\xi'$ for $\xi=\dir x{\gamma(t+???}$.
Let $\alpha(s)\df=\gexp_x(s\cdot\xi')$.
Applying ??? we get that for sutably chosen parameter $s$ we 
\[\]

the proof of inequality reminds the argument in part \ref{SHORT.thm:doubling:concave}, but it is a bit more tricky.
...
\begin{thm}{Topological stratification}
...
\begin{thm}{Fundamental-class lemma}
Let $\spc{L}$ be a compact $m$-dimensional Alexandrov space, and $m<\infty$.
Then $\spc{L}$ has a fundamental class in Alexander--Spanier cohomology with $\ZZ/2$ coefficients; 
that is, 
\[H^n(\spc{L},\ZZ/2)=\ZZ/2.\]
...
Let $f$ be a semiconcave function on a finite-dimensional geodesic $\CBB(\kappa)$ space without boundary.
