...
The definition of boundary will use induction on dimension, and it will work \textit{only} for finite-dimensional Alexandrov spaces.
...
Define \index{corank}\emph{corank} of $\spc{A}$ (briefly, $\corank\spc{A}$) as the minimal value $k$ such that $\spc{A}$ splits isometrically as $\RR^{m-k}\times \spc{K}$;
...
%??? хорошо бы, что кто-то это критически прочитал.
...
By \ref{ex:cone-CBB}, $\Cone \Xi$ is $\Alex0$.
...
Suppose $v=t\cdot u$ and $|u|=1$.
Note that $u\in\partial\T_p$.
Therefore, we can choose a sequence of points $x_n\in \partial \spc{A}$ such that $\dir{p}{x_n}\to u$; let $r_n=\dist{p}{x_n}{}$.
...
A geodesic in $\spc{W}$ has at most one interior point in $\partial \spc{A}$ or it lies in $\partial \spc{A}$ completely.
\end{clm}
...
Since $x,y\in \partial \spc{A}$, the arc of $\hat\gamma$ beyond $y$ lies in the image of map $t\z\mapsto \GF^t_{f_x}(y)$, where $f_x=\tfrac12\cdot\distfun^2_x$.
...
The doubling theorem was generalized by the third author \cite{petrunin1997} allowing gluing nonidentical spaces and generalized further by Jian Ge and Nan Li \cite{ge-li} allowing for gluing more than two spaces.
...
The Morse lemma was originally proved by Grigory Perelman \cite{perelman1993} for a special kind of semiconcave functions made out of distance functions.
For such functions it holds for all Alexandrov spaces (possibly with nonempty boundary).
For general semiconcave functions on spaces without boundary it follows from \cite{perelman1994} where the technical tools from \cite{perelman1993} (the notion of inner product between differentials of semiconcave functions and the notion of admissible maps) were generalized to include general semiconcave functions. With those definitions all the main Morse theory results of \cite{perelman1993} follow by the same proof by using \cite[2.2]{perelman1994} instead of \cite[Lemma 1]{perelman1993}.
...
here $f$ is an arbitrary function of the form $\tfrac12\cdot \distfun_p^2$. %{\red V: should we add the condition that $E$ is closed?} A: No, it follows.}
