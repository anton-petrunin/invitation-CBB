...
\begin{itemize}
\item For a closed convex set $K\subset \EE^m$ with nonempty interior, the topological boundary of $K$ as a subset of $\EE^m$ coincides with the boundary $K$ described above.
...
\item If $\Cone\Sigma$ is an $\Alex0$ space of dimensions $\ge 2$ (by \ref{ex:finite-space-of-directions-dim} this implies that $\Sigma$ is  $\Alex1$), then
...
Note that the last condition implies that $h_\eps$ is an $o(\eps)$-sometry.
...
%V: I don't understand this. doesn't this just follow from the fact that $\bar \xi$ is a point of maximum of $\phi$?
...
Therefore, we can choose a sequence of points $x_n\in \partial \spc{A}$ such that $\dir{p}{x_n}\to v$; let $r_n=\dist{p}{x_n}{}$.
