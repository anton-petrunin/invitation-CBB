...
The definition of boundary will use induction on dimension, and it will work only for finite-dimensional Alexandrov spaces.
...
Suppose $\spc{A}$ is an $m$-dimensional Alexandrov space.
...
Note that the last condition automatically implies that $h_\eps$ is an $o(\eps)$-sometry.
...
Define \index{rank}\emph{rank} of $\spc{A}$ (briefly, $\rank\spc{A}$) as the minimal value $k$ such that $\spc{A}$ splits isometrically as $\RR^{m-k}\times \spc{K}$;
...
Let $\spc{A}$ be a finite-dimensional $\Alex1$ space of dimension $\ge 2$ with nonempty boundary $\partial\spc{A}$.
