...
Define \index{rank}\emph{rank} of $\spc{A}$ (briefly, \index{51@$\rank\spc{A}$}$\rank\spc{A}$) as the minimal value $k$ such that $\spc{A}$ splits isometrically as $\RR^{m-k}\times \spc{K}$;
...
Let $\spc{A}$ be an finite-dimensional $\Alex0$ space with nonempty boundary $\partial\spc{A}$.
