...
Conclude that geodesics in $\Alex0$ space cannot \index{bifurcating geodesics}\emph{bifurcate};
...
\parit{Proof.}
Recall that only-if part follows from \ref{ex:noncreasing} and \ref{ex:adjacent-CBB}; it remains to show the if part.
...
To prove equivalence, approximate $s$ by smooth functions by taking convolutions $s_n=s*k_n$ for a suitable sequence of kernels $k_n$.
Note that all the conditions are equivalent for $s_n$;
...
That is, if the function $f\:\II\to\RR$ is a $\lambda$-concave then
\begin{subthm}{}
