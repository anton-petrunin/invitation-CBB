...
for example, on the surface of a cube in $\EE^3$, which is an $\Alex0$ space by \ref{prop:conv-surf-CBB(0)}.
...
To prove the equivalence, approximate $s$ by smooth functions by taking convolutions $s_n=s*k_n$ for a suitable sequence of kernels $k_n$.
...
so when we write $f\:\spc{A}\to \RR$ we do not assume that $f$ is defined on the whole space $\spc{A}$.
...
One could define Alexandrov spaces as a complete \textit{length} spaces with curvature $\ge \kappa$ (in the sense of Alexandrov).
