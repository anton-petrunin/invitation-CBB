...
In this lecture we discuss several definitions of Alexandrov space.
Alexandrov's lemma works as the main tool.
...
%Sometime theses exercises are nontrivial; in this case we add a note.
...
Since increasing the opposite side in a plane triangle increases the corresponding angle (this is evident from the cosine law), 
the following expressions have the same sign:
\begin{enumerate}[(i)]
...
Also note that that since a concave function defined on an open interval  is automatically locally Lipschitz, the requirement that a $\lambda$-concave function be locally Lipschitz is superfluous for open intervals.
...
In this text we have chosen complete \textit{geodesic} spaces with curvature at least $\kappa$ as the main object of study (the $\Alex\kappa$ spaces).
Instead of the \textit{geodesic} condition, we could assume that they are \textit{length} spaces. 
This is more natural and general, also many statements can be reduced to the geodesic case.
