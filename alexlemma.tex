...
\item for any $t_0\in \II$ there is a quadratic polynomial $\ell=\tfrac\lambda2\cdot t^2+a\cdot t+b$ (it is called a \index{barrie}\emph{barrier}) that (locally) supports $s$ at $t_0$ from above;
...
Also, by Plaut's theorem, \textit{any point $p$ in $\spc{L}$ can be connected by geodesics to a G-delta dense set of points in $\spc{L}$} 
