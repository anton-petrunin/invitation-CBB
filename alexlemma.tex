...
holds, then we say that the quadruple meets \index{$\CBB$ comparison}\emph{$\CBB(0)$ comparison}.
...
Construct a geodesic space $\spc{X}$ that is not $\CBB(0)$ and satisfies the following condition: for any three points $p,x,y\in \spc{X}$ there is a geodesic $[x y]$ such that for any $z\in \left]x y\right[$
...
\footnote{ \color{red}  V: why do we need to define thin triangles in this book? I think they are only used in Exercise \ref{ex:normCBB} }
\end{thm}
...
The $\CBB$ comparison is closely related to the so-called $\CAT$ comparison, which defines \textit{upper} curvature bound in the sense of Alexandrov;
