...
In particular, it is used in the proof of its generalization \cite{alexandrov-1948} 
\footnote{\red V: I would expand here a bit and say that one of the steps in the proof is that An Alexandrov metric on a surface admits a triangulation into arbitrary small convex triangles with geodesic sides. The proof in Alexandrov's book has a mistake. It was recently fixed by Creutz and Romney in the paper "Triangulating metric surfaces". They showed that this is possible on length metric on a surface without any curvature bounds. }
that gives a complete description of intrinsic metrics on the sphere that are isometric to convex surfaces in the Euclidean space. The latter statement is fundamental in a branch of modern mathematics --- the so-called \textit{Alexandrov geometry}.

The first part is central; it is called the \textit{existence theorem}.
