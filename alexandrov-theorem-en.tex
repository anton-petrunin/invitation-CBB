...
%\footnote{ \red V: this is defined earlier in the book} Я бы оставил как есть, это же апендикс...
...
%\footnote{ \red V: this is proved in chapter 11} см. выше
...
Therefore passing from a polyhedron to its surface defines a continuous map
\[\iota\:\Phi\to \Psi.\]
...
Start with a sphere $M$ with a metric from $\Psi_n$.
Suppose $v$ and $w$ are essential vertices in $M$.
Let us cut $M$ along a geodesic from $v$ to~$w$.
This way we obtain a sphere with a hole.
The hole can be patched by a disc so that the obtained metric remains in $\Psi_n$.
...
Choosing a one-parameter family of such patches, we can get a deformation of~$M$.
...
The proof of the invariance of domain theorem can be adapted to our case since both spaces $\Phi_n$ and $\Psi_n$ are $(3\cdot n-6)$-dimensional and both look like manifolds, but, formally speaking, they are \textit{not} manifolds.
...
Choose a closed subset $Z\subset \Phi_n$ and a sequence of polyhedrons $P_1,P_2,\ldots$ in $Z$.
Assume that the sequence $\iota(P_n)$ converges in $\Psi_n$.
Passing to a subsequence, we can assume that $P_n$ converges in $\Phi$; denote the limit polyhedron by  $P_\infty$.
Since $\iota$ is continuous, $\iota(P_n)$ converges to $\iota(P_\infty)$ in~$\Psi$.
Recall that $\iota(\Phi_i)\subset\Psi_i$ for each $i$; therefore, $P_\infty\in \Phi_n$.
That is, the limit of $\iota(P_n)$ belongs to $\iota(Z)$, and hence $\iota$ is closed.
