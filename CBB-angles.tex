...
Suppose that a group $G$ acts isometrically on a proper $\Alex0$ space $\spc{L}$, and $G$ has closed orbits.
...
This question is known as \index{Hopf's conjecture}\emph{Hopf's conjecture}.
...
A point $p$ in an Alexandrov space is called \index{extremal point}\emph{extremal} if $\mangle\hinge pxy\le \tfrac\pi2$ for any hinge $\hinge pxy$ with the vertex at $p$. 
...
Grigori Perelman noticed that, after proper definitions, the same proof works in Alexandrov spaces \cite{perelman-Erdos}; thus, it proves \ref{thm:extr-point}.
...
Suppose $\SSS^1$ acts isometrically on an $\Alex1$ space $\spc{L}$ that is homeomorphic to $\SSS^3$.
