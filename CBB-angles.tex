...
Assume that group $G$ acts isometrically on a proper Alexandrov space $\spc{L}$ with nonnegative curvature and has closed orbits.
Then the quotient space $\spc{L}/G$ is an Alexandrov space with nonnegative curvature.
...
Conclude that $G$ is an Alexandrov space with nonnegative curvature.
\end{thm}
...
Suppose $\spc{L}$ is an Alexandrov space with nonnegative curvature, then so is $\spc{M}$.
\end{thm}

Theorems \ref{thm:CBB/G} and \ref{thm:submetry-CBB} admit straightforward generalizations to curvature $\ge-1$.
...
A point $p$ in a $\CBB(\kappa)$ space is called \index{extremal point}\emph{extremal} if $\mangle\hinge pxy\le \tfrac\pi2$ for any hinge $\hinge pxy$ with the vertex at $p$. 
...
\index{properly discontinuous}\emph{properly discontinuous} (that is, for any compact set $K\subset \EE^m$ and $x\z\in \EE^m$ there are only finitely many $g\in \Gamma$ such that $g\cdot x\in K$) and \emph{cocompact} (that is, the quotient space $\spc{L}=\EE^m/\Gamma$ is compact).
...
An extremal subset is called \index{primitive extremal set}\emph{primitive} if it contains no proper extremal subsets.
