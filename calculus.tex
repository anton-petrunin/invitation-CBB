...
This lecture introduces several notions related to the first-order derivatives in Alexandrov spaces, including spaces of directions, tangent spaces, differentials, and gradients.
...
\]
The value $\mangle(v,w)$ is undefined if $v=\0$ or $w=\0$;
...
Let $f$ be a semiconcave function on an Alexandrov space $\spc{A}$ (recall that this means in particular that $f$ is locally Lipschitz on $\Dom f$), and $p\z\in \Dom f$.
...
The main difficulty lies in the proof of \ref{eq:2|wt|=|ut+vt|};
solving the following exercise should give a hint.
...
Existence of $\overline{\xi}$ is obvious if  $ \Sigma_p$. In the general case take a sequence of directions $\xi_n\in \Sigma_p$, such that $(\dd_p f)(\xi_n)\to s$.
...
Here we assume that all functions $f_1$, $f_2,\dots$ are semiconcave and uniformly locally Lipschitz near $p$. 
\end{thm}
