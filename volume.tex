...
This lecture discusses main source of applications of Alexandrov geometry.
...
By \ref{thm:right-inverse}, $\infty>\vol \spc{A}>0$.
...
In the proof we consider a sum $\phi\circ f_1+\ldots+\phi\circ f_N$ for suitably chosen sequence of functions $f_1,\ldots,f_N$, so that for any geodesic $\gamma$ in a small neighborhood of $p$ the value $|(f_i\circ\gamma)^\pm|$ is bounded away from zero for some $i$.
...
there is no open set $\Omega\subset \spc{A}$ with strictly concave function $f\:\Omega\to\RR$.
\end{thm}
...
\begin{itemize}
\item Arguing by contradiction, we assume existence of a sequence of manifolds that violates the statement.
