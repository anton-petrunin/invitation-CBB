...
Let $f$ be a Lipschitz real-to-real function, and let $\phi$ be a smooth function defined in a neighborhood of $x_0=f(t_0)$.
...
In particular, $p$ is the maximal point of $f$.
...
Applying this theorem with Gromov's selection theorem (\ref{thm:gromov-compactness}) and Exercise~\ref{ex:pack-vol:dim}, we get the following.
...
A more exact version of this construction is given in the survey of the third author \cite[7.2]{petrunin:survey}.
...
Furthermore, such a lifted function has the following property (let us call it \index{gap-convexity}\emph{gap-convexity}) which, as far as we know, did not find an application:
\textit{If $f$ is a controllably concave function and $f_n$ are liftings of $f$ to a collapsing sequence of Alexandrov spaces with uniform lower curvature bound, then for any $\eps>0$ and all large $n$, the restriction of $f_n$ to any geodesic of length at least $\eps$ is concave.}
