...
In particular, we may choose small $\eps>0$, so that for any point $p\in \spc{L}$, there is a point $p'\in \spc{L}'$ such that $\dist{p}{p'}{\spc{W}}<\eps$;
...
Applying several results in differential topology,\footnote{\red A: Vitya could you give refs?} this statement can be improved to diffeomorphism-type finiteness in all dimensions except 4.
