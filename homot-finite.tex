...
For smooth real-to-real function  functions  $f, \phi$ this follows from the chain rule $\phi(f(x)''=\phi''(f(x))\cdot (f'(x))^2+\phi'(f(x)) \cdot f''(x)$.
...
Suppose $r>0$ small and $c$ is large (these values will be specified a bit later).
...
is a strictly concave in a neighborhood of $p$.
\end{thm}
...
Functions with controlled concavity can be lifted to a collapsing sequence, in this case the resulting function is not concave but it has many properties.
...
Furthermore, these liftings have the following peculiar property (let us call it \index{gap-convexity}\emph{gap-convexity}) which, as far as we know, did not find an application:
\textit{If $f$ is controllably concave unction and $f_n$ are liftings of $f$ to a collapsing sequence of Alexandrov spaces with uniform lower curvature bound, then for any $\eps>0$, and all large $n$, the restriction of $f_n$ to any geodesic of length at least $\eps$ is concave.}
