...
Now, assume $\{q_1,\dots, q_N\}$ is a finite set of points such that $\dist p{q_i}{}=r$ for every $i$.
For a geodesic $\gamma$, set $\alpha_i(t)=\mangle(\gamma^+(t),\dir {\gamma(t)}{q_i})$. 
Assume that
\[\max_i\{|\alpha_i(t)-\tfrac\pi2|\}\ge\eps>0\eqlbl{eq:angle-ne-pi/2}\]
for any geodesic $\gamma$ in $\oBall(p,\eps)$. 
Choose $c>3N/\cos^2\eps$;
...
Therefore, for small $\delta>0$ and yet smaller $\eps>0$, the set $\{q_i\}$ meets \ref{eq:angle-ne-pi/2}.
...
then we can be repeated in $\spc{A}'$ for the liftings of all points and the same function $\phi$.
...
\begin{itemize}
\item Arguing by contradiction, we assume existance of a sequence of manifolds that violates our assumption.
\item Use Gromov's selection theorem to choose a converging subseqence.
...
However, it is easy to produce a proof following the steps above (noone bothered to write it so far).
...
for more exact statements can see \cite{kapovitch2002} and \cite{kapovitch2005}.
