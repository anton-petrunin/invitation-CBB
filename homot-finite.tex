...
Moreover, in the only-if part, we can assume in addition, that $\bar f'(t_0)$ takes a given value in the interval $[f^+(t_0),f^-(t_0)]$.
\end{thm}
...
Note that $s>0$ and $f(t_0)=\tfrac s2\cdot (t_0-a)\cdot(t_0-b)$;
...
Suppose that $f$ is $\lambda_1$-concave for some $\lambda_1\ge 0$ and there are some constants $c_1,c_2,c_3\in\RR$ such that (1) $|f^+(t_0)|>c_1>0$ (or $|f^-(t_0)|>c_1>0$), (2) $c_2>\phi'(x_0)\ge 0$, and (3) $c_3\ge \phi''(x_0)$.
Let $\lambda_2=c_1\cdot\lambda_1+c_2^2\cdot c_3$.
Show that $\phi\circ f$ is $\lambda_2$-concave in a neighborhood of~$t_0$.
\end{thm}

\begin{thm}[!]{Exercise}\label{ex:pack-sphere}
Given a point $p$ in an $m$-dimensional Alexandrov space $\spc{A}$ and a nonempty open set $\Omega$ there are $r>0$ and $\Const>0$ such that for any sufficiently small $\eps>0$ there are at least $\Const/\eps^{m-1}$ points $q_1,\dots,q_N\in \Omega$ such that $\dist{p}{q_i}{}=r$ for any $i$ and $\mangle\hinge p{q_i}{q_j}>\eps$ if $i\ne j$.
\end{thm}
...
we can choose $N\ge \Const_1/\eps_3^{m-1}$ points $\{q_i\}$ such that $\dist{p}{q_i}{}=r$ for some small $r>0$
...
for more exact statements, see \cite{kapovitch2002} and \cite{kapovitch2005}.
