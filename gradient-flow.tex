...
The tangent vector $v=\dist px{}\cdot\dir px$ can be called the \index{logarithm}\emph{logarithm} of $x$ at $p$ (briefly, \index{$\log_p x$ (logarithm)}$\log_p x$);
...
Let $f\:\spc{A}\subto\RR$ be a locally Lipschitz and semiconcave function on an Alexandrov space
$\spc{A}$.
...
Then for any $p\in \Dom f$, there are unique $t_{\max}\in(0,\infty]$ and $f$-gradient curve $\alpha\:[0,t_{\max})\to \spc{A}$ with $\alpha(0)=p$ such that any sequence $t_n\to t_{\max}-$, the sequence $\alpha(t_n)$ does not have a limit point in $\Dom f$.
\end{thm}

This theorem says that the future of a gradient curve is determined by its present, but it says nothing about its past.
...
that converges to $f_\infty\:\spc{A}\z\to \RR$. 
...
Let $\spc{A}$ be an Alexandrov space and $p\in \spc{A}$, consider the function $f\z=\distfun_p^2/2$.
...
The idea to use gradient flow in Alexandrov geometry was inspired by the success of \index{Sharafutdinov's retraction}\emph{Sharafutdinov's retraction} in comparison geometry \cite{sharafutdinov}.
...
For a general curvature bound $\kappa$, the construction of gradient exponent has to be modified;
