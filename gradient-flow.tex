...
In general the logarithm is a multivalued function from $\spc{A}$ to $\T_p$; so, $v=\log_p x$ and $w=\log_p x$ does \textit{not} mean $v=w$.
...
The gradient exponential map in a Riemannian manifold $(M,g)$ coincides with the Riemannian exponential map inside the cut locus, but \textit{is different} from the Riemannian exponential after that.
