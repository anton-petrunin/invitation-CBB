...
The tangent vector $v=\dist px{}\cdot\dir px$ will be called the \index{logarithm}\emph{logarithm} of $x$ at $p$ (briefly, \index{$\log_p x$ (logarithm)}$\log_p x$).
...
Let $f\:\spc{A}\subto\RR$ be a semiconcave function on an Alexandrov space
$\spc{A}$.
...
Let $\spc{A}$ be an Alexandrov space of curvature $\ge 0$  and $p\in \spc{A}$, consider the function $f\z=\distfun_p^2/2$.
