...
Let $\spc{L}$ be an Alexandrov space and $p\in \spc{L}$, consider the function $f\z=\distfun_p^2/2$.
...
The idea to use gradient flow in Alexandrov geometry was inspired by the success \index{Sharafutdinov's retraction}\emph{Sharafutdinov's retraction} in comparison geometry \cite{sharafutdinov}.
The gradient flow was developed to prove the existence of quasigeodesics with given initial data \cite{perelman-petrunin:qg,petrunin:qg, petrunin:survey}, but it turned out that gradient flow and gradient exponent are a more useful technical tools in Alexandrov geometry.
Very soon these tools were developed and found applications in other singular spaces \cite{jost,mayer,lytchak:open-map,ohta,sevare,ambrosio-gigli-savare}.
