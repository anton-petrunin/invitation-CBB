...
Let $\spc{W}$ be a vector space with distance given by a norm.
Show that if all angles between geodesics in $\spc{W}$ are defined, then the norm is induced by an inner product.
In particular, if $\spc{W}$ is finite-dimensional, then it is isometric to $\EE^n$ for some finite $n$.
\end{thm}
