...
Most importantly, the reader should know and love elementary geometry, including convexity.
...
$\trig p q r=\trig p q r_{\spc{X}}=([q r], [r p], [p q])$\index{10@$\trig p q r=\trig p q r_{\spc{X}}$ (triangle)}.
...
Show that any surjective non-expanding map from a compact metric space to itself is an isometry.
\end{thm}

For the proof of the following statements we refer to \cite{burago-burago-ivanov} and \cite{petrunin2023pure}.
...
\begin{subthm}{}
$f(\spc{X})$ is an \index{net@$\eps$-net}\emph{$\eps$-net} in $\spc{Y}$; that is, for any $y\in \spc{Y}$ there is $x\in \spc{X}$ such that $\dist{f(x)}{y}{\spc{Y}}<\eps$.
\end{subthm}
