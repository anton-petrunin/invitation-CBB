...
\item $\MM^m(0)=\EE^m$,
\item if $\kappa<0$, then $\MM^m(\kappa)$ is the Lobachevsky $m$-space $\HH^m$ rescaled by factor $\tfrac{1}{\sqrt{-\kappa}}$;
...
In particular, these convergences are different, but if we restrict our attention only to nonempty uniformly bounded compact spaces, then these two convergences are essentially the same.
Let us sketch why.
