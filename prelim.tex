...
\begingroup
  \def\@currentlabel{}%
  \def\@currentlabelname{Preliminaries}%
  \label{chap:prelim}%
\endgroup
\makeatother
...
The \index{pack@$\eps$-pack}\emph{$\eps$-pack} of $\spc{X}$ (or \index{packing number}\emph{packing number}) is the maximal number  (possibly infinite) of points in $\spc{X}$ at distance $>\eps$ from each other;  it is denoted by $\pack_\eps\spc{X}$\index{70@$\pack_\eps\spc{X}$ ($\eps$-pack)}.
