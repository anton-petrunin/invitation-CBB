...
Let $\spc{X}$ be a metric space,
and let $\II$\index{$\II$ (real interval)} be a real interval.
...
A metric space $\spc{X}$ is called \index{$\ell$-geodesic space}\emph{$\ell$-geodesic}
...
\index{$\modtrig$ (model triangle)}) to be a triangle in the Euclidean plane $\EE^2$ with the same sides;
that is,
\[
