...
Let $\spc{X}_1,\spc{X}_2,\dots$, and $\spc{X}_\infty$ be compact metric spaces, and let $\eps_n\to\0+$ as $n\to\infty$.
Suppose that either 
...
\\
\dist{x_n}{x_m}{\bm{X}}&=\inf\set{\rho(x_n,y_\infty)+\rho(x_m,y_\infty)}{y_\infty\in\spc{X}_\infty},
\end{align*}
where we assume that $x_\infty,y_\infty\in \spc{X}_\infty$, and $x_n,y_n\in \spc{X}_n$ for each $n$. 
It remains to observe that this indeed defines a metric on $\bm{X}$, and $\spc{X}_n\Hto \spc{X}_\infty$ in the sense of Hausdorff.
