...
When dealing with Gromov-Hausdorff convergence the following Lemma is often useful as it allows to bypass constructing explicit metrics on the disjoint unions   $\bigsqcup \spc{X}_n$ 
...
Furthermore, the converse also holds, that is

\begin{subthm}{lem:almost-isom-c}
Suppose we have a Gromov--Hausdorff convergence $\spc{X}_n\GHto \spc{X}_\infty$. Then there exist $\eps_n\to\0+$ as $n\to\infty$ and  $\eps_n$-isometris   $f_n\:\spc{X}_n\to\spc{X}_\infty$  and $h_n\:\spc{X}_\infty\to\spc{X}_n$ 
such that $x_n\in \spc{X}_n$ converges to $x_\infty \in  \spc{X}_\infty$ with respect to the  convergence $\spc{X}_n\GHto \spc{X}_\infty$ iff $f_n(x_n)\to x_\infty$ and iff $\dist{h_n(x_\infty) }{x_n}{\spc{X}_n}\to 0$ as $n\to\infty$.
\end{subthm}
\end{thm}
...
We leave part \ref{SHORT.lem:almost-isom-c} as an easy exercise for the reader.
\qedsf


Lemma ~\ref{lem:almost-isom} has a natural analogue for pointed convergence. For simplicity we only state part (a) of the Lemma. Parts (b) and (c) can be rephrased similarly.
...
\begin{subthm}{}
$\bigl|\dist{f_n(x)}{f_n(x')}{\spc{X}_\infty }-\dist{x}{x'}{\spc{X}_n}\bigr|\le \eps(n,R)$ for any $x,x'\in \oBall(p_n,R)$.
\end{subthm}
...
Then there is a pointed  Gromov--Hausdorff convergence $\spc{X}_n\GHto \spc{X}_\infty$.


\end{thm}
The proof of Lemma \ref{lem:almost-isom-pointed} is analogous to the proof of Lemma ~\ref{lem:almost-isom}  and is left to the reader as an exercise.
