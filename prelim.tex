...
Furthermore, we define distance function to a subset $A\subset \spc{X}$ as
...
that is,
\begin{itemize}
\item if $\kappa>0$, then $\MM^m(\kappa)$ is the $m$-sphere of radius $\tfrac{1}{\sqrt{\kappa}}$, so $\SSS^m=\MM^m(1)$
\item $\MM^m(0)=\EE^m$,
...
Suppose $A$ and $B$ are two compact subsets of a metric space $\spc{X}$ Then $|A-B|_{\Haus\spc{X}}< R$ if and only if
...
Loosely speaking, the distance between (isometry classes of) compact metric spaces will be defined as the maximal value $r$ such that  both spaces can be isometrically embedded into a third space in  such a way that the Hausdorff distance between the images is not greater than $r$.
...
Given  $r > 0$, we says that $|\spc{X}-\spc{Y}|_{\GH} < r$ if and only if there exists a metric
