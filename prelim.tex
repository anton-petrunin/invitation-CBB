...
Loosly speaking, it the distance between (isometry classes of) compact metric spaces will be defined as the maximal value such that \textit{the distance between subspaces in a metric space is not greater than the Hausdorff distance between them}.

Observe that we \textit{know} a real number $x$ if we can answer yes or no question $x<r$ for any $r$.
Keep this in mind while readint the following definition.
...
These distances spaces actually define a metric on (isometry classes of) compact metric spaces; for a proof we refer to \cite{burago-burago-ivanov,petrunin2023pure}.
The obtained metric is called the Gromov--Hausdorff metric;
the corresponding metric space will be denoted by $\GH$.
Moreover, each of these textbooks \cite{burago-burago-ivanov,petrunin2023pure} provide the following statement.
...
One is defined in the previous section; it works for arbitrary metric spaces and
its limit might not be uniquely defined.
And the other is defined as convergence in $\GH$;
the limit in $\GH$ is unique, if it exists.
Therefore, these convergences are different.
On the other hand, if we restrict our attention only to compact spaces, then these two convergences are essentially the same.
...
Assume $\spc{X}_\infty$ is compact; show that $\spc{X}_n$ converges to $\spc{X}_\infty$ in $\GH$.
Conclude that $\spc{X}_\infty$ is isometric to~$\spc{X}_\infty'$.

\end{thm}

The following statement is primarily responsible for the properties of convergence of compact spaces.
...
\begin{subthm}{}
$f(\spc{X})$ is an \index{distance-preserving}\emph{$\eps$-distance-preserving}; that is,
