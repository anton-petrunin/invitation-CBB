...
One can try to define Gromov-Hausdorff distance between complete metric spaces which are not necessarily compact. This is not usually done because the following phenomena can occur

\begin{thm}{Exercise}\label{ex:GH-noncompact}
Construct two complete geodesic metric spaces $X, Y$ such that $|\spc{X}-\spc{Y}|_{\GH}=0$ but $X$ is not isometric to $Y$.
\end{thm}
