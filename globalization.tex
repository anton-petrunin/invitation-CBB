...
Otherwise note that the sequence $r_n=\dist{p}{x_n}{}+\dist{x_n}{q}{}$ is nonincreasing, and $r_n\ge\tfrac{2}{3}\cdot\ell$ for all $n$.  Since  $\eps_n=r_n-r_{n+1}\to 0$ we can assume that $\eps_n\le \ell/1000$ for all $n$.
We claim that the distances
$\dist{x_n}{x_{n+1}}{}$, $\dist{x_n}{p}{}$, and $\dist{x_n}{q}{}$ are bounded away from zero for all large $n$.

First note that $\dist{x_n}{x_{n+1}}{}\ge \ell/200$ for all large $n$.  If not then by  \ref{3|xx'|}  we have that $\dist{x_n}{p}{}\ge \tfrac{2}{3}\cdot\ell-3\ell/200 $,  $\dist{x_n}{q}{}\ge \tfrac{2}{3}\cdot\ell-3\ell/200 $ and hence $\dist{x_n}{p}{}+\dist{x_n}{q}{}\ge \tfrac{4}{3}\cdot\ell-6\ell/200>\ell $ which contradicts  \ref{eq:thm:=def-loc-two-sided*}.


We claim that $\dist{x_n}{p}{}\ge \ell/200$  and $\dist{x_n}{q}{}\ge \ell/200$  for all large $n$ also. We will prove this by induction.
It easily follows from  \ref{3|xx'|} and  \ref{eq:thm:=def-loc-two-sided*} and  that if  $\dist{x_{n-1}}{p}{} <\ell /200$ or $\dist{x_{n-1}}{q}{}< \ell/200$  then $\dist{x_n}{p}{}\ge \ell/200$ and $\dist{x_n}{q}{}\ge \ell/200$. 
Next we want to show that the same lower bound holds for $x_{n+1}$. 

Without loss of generality we can assume that $ \dist{x_n}{p}{}\le  \dist{x_n}{q}{}$. As before set $x=x_n, x'=x_{n+1}$.

We have $\eps_n= \dist{x}{p}{}+ \dist{x}{x'}{}-\dist{x'}{p}{}, \dist{x'}{p}{}= \dist{x}{p}{}+ \dist{x}{x'}{}-\eps_n\ge \ell/200+\ell/200-\ell/1000\ge \ell/200$.

Suppose  $\dist{x'}{q}{}\le \ell/200$.

Then from  \ref{eq:thm:=def-loc-two-sided*}  we have

\[
\tfrac{2}{3}\cdot\ell \le\dist{p}{x}{}\z+\dist{x}{q}{}=\dist{p}{x}{}\z+\dist{x}{x'}{}+\dist{x'}{q}{}\le \dist{p}{x}{}\z+\dist{x}{x'}{}+\ell/200
\]
Subtracting this from  \ref{3|xx'|} we get

\[
0\ge 2\dist{x}{x'}{}-\ell/200,\quad \dist{x}{x'}{}\le \ell/400
\]
which contradicts that  $ \dist{x}{x'}{}\ge \ell /200$.
Now the desired claim follows by induction.
...
\footnote{\red V: this should be explained in more detail either in this book or in the big one. whenever I prove the key lemma none of the students can fill this step on their own.   }
