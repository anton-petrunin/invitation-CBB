...
this is the opposite side in a plane triangle with the same angle and adjacent side lengths as in $\hinge x p q$.
...
First, let us describe a construction of a new hinge $\hinge{x'}p q$, which has several properties described below;
in particular,
\[
...
Set $x_0=x$.
Applying the above construction inductively, we can construct a sequence of hinges $\hinge{x_n}p q$ with $x_{n+1}\z=x_n'$.
...
Since $x_n$ does not terminate, we have $\tfrac{2}{3}\cdot\ell\le r_n<\ell$ for all $n$.
...
A proof can be found in~\cite[2F]{alexander-kapovitch-petrunin2024}; it is based on the idea from \ref{thm:glob}, but requires a few additional twists that we are about to discuss.
...
=\pi.
\]
Use this example as a warning when you work with a non-intrinsic metric.
