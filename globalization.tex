...
The sequence $x_n$ might terminate at some $n$ only if
...
From \ref{eq:thm:=def-loc-two-sided*} and \ref{eq:thm:=def-loc-fourstar}, $a_n\le \tfrac12\cdot\ell$.
Hence
\[
\tfrac29\cdot\ell\ge \dist{x_n}{x_{n+1}}{}\ge \tfrac1{18}\cdot\ell.
\eqlbl{eq:|xx'|>=l/18}
\]
In particular, we get the claim for $\dist{x_n}{x_{n+1}}{}$.

Note that we have one of the following
\[
\begin{split}
a_{n+1}&=b_n-\dist{x_n}{x_{n+1}}{}\qquad \text{or}
\\
a_{n+1}&=r_{n+1}-b_n+\dist{x_n}{x_{n+1}}{}=
\\
&=a_n-(r_n-r_{n+1})+\dist{x_n}{x_{n+1}}{}.
\end{split}
\]
Since $r_n$ does not increase, it must converge;
in particular, $r_n-r_{n+1}\z\to 0$ as $n\to\infty$.
...
Since $r_n-r_{n+1}\to 0$, we have that $\angk{x_n}{p_n}{x_{n+1}}\to \pi$,
