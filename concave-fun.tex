...
Let $\spc{L}$ be an Alecandrov space.
By \ref{clm:angle-defined}, angle measure of any hinge in is defined.
...
that is, $\mangle$ meets all conditions in the definition of metric except it might vanish for distinct elements. 

The metric space corresponding to  $(\mathfrak{S}_p,\mangle)$ is called the \index{space of geodesic directions}\emph{space of geodesic directions} at $p$, denoted by $\Sigma'_p$ or $\Sigma'_p\spc{L}$.
...
The completion of $\Sigma'_p$ is called the \index{space of directions}\emph{space of directions} at $p$ and is denoted by $\Sigma_p$ or $\Sigma_p\spc{L}$.
...
Show that $\Cone\spc{X}$ is geodesic if and only if $\spc{X}$ is \index{geodesic space!$\pi$-geodesic}\emph{$\pi$-geodesic};
that is any two points $x,y\in \spc{X}$ such that $\dist{x}{y}{\spc{X}}<\pi$ can be joined by a geodesic in $\spc{X}$.
\end{thm}

\parbf{Tangent space.}
The Euclidean cone $\Cone\Sigma_p$ over the space of directions $\Sigma_p$ is called the \index{tangent space}\emph{tangent space} at $p$ and denoted by $\T_p$ or $\T_p\spc{L}$.
The elements of $\T_p\spc{L}$ will be called \index{tangent vector}\emph{tangent vectors} at $p$
...
Halbeisen's example \cite{alexander-kapovitch-petrunin2024} shows that tangent space $\T_p$ to an Alexandrov space might fail to be geodesic;
...
Let $\spc{L}$ be an Alexandrov space.
Let $f$ be a semiconcave function on $\spc{L}$ and $p\in \Dom f$.
...
The following definition generalizes gradient to semiconcave functions on Alexandrov space.
This generalization is not trivial even for concave functions on Euclidean space;
we suggest to keep this case in mind.
...
\end{figure}
\footnote{\red V: I would like to suggest redoing this picture so that it works for $\lambda=0$ and contracts the distances in first order. that gives the right intuition.}
...
\[\dd_p\distfun_q(v)\le -\langle\dir pq,v\rangle\]
for any $v\in\T_p$.
\footnote{\red V: I am confused. this is just \ref{ex:d(distfun):<} which is trivial by Toponogov and does not require the geodesic to be extendible }
\end{thm}

\parit{Proof of \ref{lem:ohta}.}
We will assume that $\spc{L}$ is $\Alex0$ and $f$ is concave;
